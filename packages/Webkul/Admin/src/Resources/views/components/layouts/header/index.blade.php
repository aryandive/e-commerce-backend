...
<header
    class="sticky top-0 z-[10001] flex items-center justify-between border-b bg-white px-2 py-2 dark:border-gray-800 dark:bg-gray-900 sm:px-4 sm:py-2.5">
    <div class="flex items-center gap-1 sm:gap-1.5">
        <!-- Hamburger Menu -->
        <i class="icon-menu cursor-pointer rounded-md p-1.5 text-xl hover:bg-gray-100 dark:hover:bg-gray-950 lg:hidden sm:text-2xl"
            @click="$refs.sidebarMenuDrawer.open()">
        </i>

        <!-- Logo -->
        <a href="{{ route('admin.dashboard.index') }}" class="flex-shrink-0 flex items-center">
            <img src="{{ asset('images/company-logo.webp') }}"
                class="h-8 w-auto sm:h-10 transition-all duration-200 invert dark:invert-0" id="logo-image"
                alt="{{ config('app.name') }}" />
        </a>
        ...
        <v-mega-search class="hidden sm:block">
            <div
                class="relative flex w-[200px] items-center sm:w-[300px] md:w-[400px] lg:w-[525px] xl:max-w-[525px] ltr:ml-2 rtl:mr-2 sm:ltr:ml-2.5 sm:rtl:mr-2.5">
                <i
                    class="icon-search absolute top-1.5 flex items-center text-xl ltr:left-2 rtl:right-2 sm:text-2xl sm:ltr:left-3 sm:rtl:right-3"></i>
    ...
    <div class="flex items-center gap-1 sm:gap-2.5">
        ...
        <v-dark>
            <div class="flex">
                <span
                    class="{{ request()->cookie('dark_mode') ? 'icon-light' : 'icon-dark' }} cursor-pointer rounded-md p-1.5 text-xl transition-all hover:bg-gray-100 dark:hover:bg-gray-950 sm:text-2xl"></span>
            </div>
        </v-dark>

        <!-- Visit Shop Link -->
        <a href="{{ route('shop.home.index') }}" target="_blank" class="hidden sm:flex">
            <span
        ...
        <x-admin::dropdown position="bottom-{{ core()->getCurrentLocale()->direction === 'ltr' ? 'right' : 'left' }}">
            <x-slot:toggle>
                @if ($admin->image)
                    <button
                        class="flex h-8 w-8 cursor-pointer overflow-hidden rounded-full hover:opacity-80 focus:opacity-80 sm:h-9 sm:w-9">
                        <img src="{{ $admin->image_url }}" class="h-full w-full object-cover" />
                    </button>
                @else
                    <button
                        class="flex h-8 w-8 cursor-pointer items-center justify-center rounded-full bg-blue-400 text-xs font-semibold leading-6 text-white transition-all hover:bg-blue-500 focus:bg-blue-500 sm:h-9 sm:w-9 sm:text-sm">
                ...
                <x-slot:content class="!p-0">
                    <div
                        class="flex items-center gap-1.5 border border-b-gray-300 px-4 py-2 dark:border-gray-800 sm:px-5 sm:py-2.5">
                        <img src="{{ url('cache/logo/bagisto.png') }}" class="sm:h-6 sm:w-6" width="20" height="20" />
                    ...
                    <div class="grid gap-1 pb-2.5">
                        <a class="cursor-pointer px-4 py-2 text-sm text-gray-800 hover:bg-gray-100 dark:text-white dark:hover:bg-gray-950 sm:px-5 sm:text-base"
...
<!-- Menu Sidebar Drawer -->
<x-admin::drawer position="left" width="270px" ref="sidebarMenuDrawer">
    <!-- Drawer Header -->
    <x-slot:header>
        <div class="flex items-center justify-between">
            @if ($logo = core()->getConfigData('general.design.admin_logo.logo_image'))
                <img src="{{ Storage::url($logo) }}" class="h-8 w-auto sm:h-10" alt="{{ config('app.name') }}" />
            @else
                <img src="{{ request()->cookie('dark_mode') ? bagisto_asset('images/dark-logo.svg') : bagisto_asset('images/logo.svg') }}"
                    class="h-8 w-auto sm:h-10" id="logo-image" alt="{{ config('app.name') }}" />
            @endif
        </div>
        </x-slot>
                ...
                <nav class="grid w-full gap-1.5 sm:gap-2">
                    ...
                    @foreach (menu()->getItems('admin') as $menuItem)
                        <div class="group/item relative">
                            <a href="{{ $menuItem->getUrl() }}"
                                ...
                                <span
                                    class="{{ $menuItem->getIcon() }} text-xl {{ $menuItem->isActive() ? 'text-white' : ''}} sm:text-2xl"></span>

                                <p
                                    class="font-semibold text-gray-600 dark:text-gray-300 whitespace-nowrap text-sm group-[.sidebar-collapsed]/container:hidden {{ $menuItem->isActive() ? 'text-white' : ''}} sm:text-base">
                                    {{ $menuItem->getName() }}
                                </p>
                            </a>
                            ...
                            @if ($menuItem->haveChildren())
                                <div
                                    class="{{ $menuItem->isActive() ? ' !grid bg-gray-100 dark:bg-gray-950' : '' }} hidden min-w-[180px] ltr:pl-8 rtl:pr-8 pb-2 rounded-b-lg z-[100] sm:ltr:pl-10 sm:rtl:pr-10">
                    ...
                    <div class="group/item relative">
                        <a href="{{ route('admin.help.index') }}"
                            ...
                            <svg class="h-5 w-5 shrink-0 {{ $isHelpActive ? 'text-white' : 'text-gray-600 dark:text-gray-300' }} sm:h-6 sm:w-6"
                            ...
                            <p
                                class="font-semibold text-gray-600 dark:text-gray-300 whitespace-nowrap text-sm {{ $isHelpActive ? 'text-white' : '' }} sm:text-base">
